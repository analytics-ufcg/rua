...
information <- function(){
	...
	for(i in fileNames){
		...
		dados$CPU_UTIL <- (dados$CPU_UTIL - min(na.omit(dados$CPU_UTIL))) / (max(na.omit(dados$CPU_UTIL)) - min(na.omit(dados$CPU_UTIL)))
		dados$MEM_UTIL <- (dados$MEM_UTIL - min(na.omit(dados$MEM_UTIL))) / (max(na.omit(dados$MEM_UTIL)) - min(na.omit(dados$MEM_UTIL)))
		...
		reg = lm(dados$MEM_UTIL ~ dados$CPU_UTIL)
		retorno$Rlinear[contador] <- summary(reg)$r.squared
		
		reg = lm(dados$MEM_UTIL~dados$CPU_UTIL+I(dados$CPU_UTIL^2))
		retorno$Rpoli2[contador] <- summary(reg)$r.squared
		
		reg = lm(dados$MEM_UTIL~dados$CPU_UTIL+I(dados$CPU_UTIL^2)+I(dados$CPU_UTIL^3))
...
write.csv(resultado,"informações.csv", row.names=FALSE)