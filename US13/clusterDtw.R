...
geraMatrizDist <- function(){
  ...
  matriz <- dist(c(1:5), method = "euclidean")
  contador = 1
  ...
  for(i in 1:(length(fileNames)-1)){
    ...
    for(j in (i+1):length(fileNames)){
      ...
      matriz[contador][1] = sum(dist(serieA[1:4000], serieB[1:4000], method="DTW"))
      contador = contador + 1
    }
  }
  
  grupos <- kmeans(matriz, centers = 3)
...
#Normaliza̤̣o/Elimina̤̣o de ruidos
trace1$CPU_UTIL <- ceiling(trace1$CPU_UTIL)
